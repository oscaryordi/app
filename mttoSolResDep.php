...
$cuenta_gps 		= " SELECT id_mttoSol FROM mttoSol WHERE capturo = '$id_usuario' AND pagado = 0 AND id_prov != 81 AND rbolso = 0  ";
}

if($_SESSION["mttoSol"] > 2){
$cuenta_gps 		= " SELECT id_mttoSol FROM mttoSol WHERE pagado = 0  AND id_prov != 81 AND rbolso = 0 ";	
}
...
// SI CONSULTA EJECUTIVO
	$sql_mttoSol = 'SELECT * '
        . ' FROM mttoSol '
        . "  WHERE capturo = '$id_usuario' AND pagado = 0 AND id_prov != 81 AND rbolso = 0 "
        . ' ORDER BY '
        . ' id_mttoSol '
        . ' DESC '
...
// SI CONSULTA SUPERVISOR
	$sql_mttoSol = 'SELECT * '
        . ' FROM mttoSol '
        . "  WHERE  pagado = 0 AND id_prov != 81 AND rbolso = 0 "
        . ' ORDER BY '
        . ' id_mttoSol '
        . ' DESC '
        . " LIMIT $pagina_1, $rxpag " ;
...
for($i=$min_mostrar; $i <= ($max_mostrar); $i++) 
	{
		...
		echo "<a href='mttoSolResDep.php?pagina=$i' style='color:$color;text-decoration: none; padding: 0px 7px; margin: 0px 2px; background-color: #F8F8F8 ;' >$i</a> ";
	}
