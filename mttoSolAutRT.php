...
$cuenta_gps 		= " SELECT id_mttoSol FROM mttoSol WHERE capturo = '$id_usuario' AND cancelado = 0 ";
}

if($_SESSION["mttoSol"] > 2){
$cuenta_gps 		= " SELECT id_mttoSol FROM mttoSol  WHERE cancelado = 0 ";	
}
...
// SI CONSULTA EJECUTIVO
	$sql_mttoSol = 'SELECT * '
        . ' FROM mttoSol '
        . "  WHERE capturo = '$id_usuario'  AND cancelado = 0 "
        . ' ORDER BY '
        . ' id_mttoSol '
        . ' DESC '
...
for($i=$min_mostrar; $i <= ($max_mostrar); $i++) 
	{
		...
		echo "<a href='mttoSolAutRT.php?pagina=$i' style='color:$color;text-decoration: none; padding: 0px 7px; margin: 0px 2px; background-color: #F8F8F8 ;' >$i</a> ";
	}
