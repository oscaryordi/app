...
$preAutorizado = "<i 	class='far fa-smile'  
						style='font-size:16px; color:orange; font-weight: bold;'   
						alt='PRE-AUTORIZADO' >
				  </i>";
...
$contratosArray[] 	= ''; // se inicializa fuera del ciclo // optimizacion
$clientesArray[] 	= ''; // se inicializa fuera del ciclo // optimizacion
$provArray[] 		= ''; // se inicializa fuera del ciclo // optimizacion
...
// INICIO poner renglon resultados
	...
	if($_SESSION['almacen']>0){
		if($id_sucursal != 0){sucursalxid($id_sucursal);}
	}
	echo "<td>{$esrbolso}{$nombreR}{$esrbolso2}".$provArray[$id_prov][1]." <h4>$nombreSucursal</h4></td>";
	...
	// SUPER PERMISO ITZEL GODOY
	if($capturo == 497 OR $_SESSION['id_usuario'] ==  11 )
		{
...
else
{
	...
	if($dM5 > 0)
		{
			echo "<td>";
			for($x = 1; $x <= $dM5; $x++){ 
			echo "<a href='mttoSolVerDocto.php?id_mtto=$id_mttoSol&tipo=5&nodoc=$x'  
	...
	if($pagadoInfo == 1)
		...
		if($bancoE=="3") 
			{ //Es mary
			$pagadoInfoMostrar = "<a href='#' ><span class='pagadoInfoM' title='PAGADO EN BLOQUE, CREDITO' >P.C.</span></a>";	
			}	
			//$pagadoInfoMostrar = "<a href='http://jetvan.ddns.net:8021/pdfpagos/getfile.php?uuid=".$id_mttoSol."'  target='_blank'  ><span class='pagadoInfoM' title='VER AVISO DE PAGO' >$</span></a>";
		}
	...
	if($pagado > 0)
		{
			...
			for($x = 1; $x <= $pagado; $x++){ 
			echo "
				<a href='mttoSolVerDocto.php?id_mtto=$id_mttoSol&tipo=2&nodoc=$x'  
	...
	// SEGUIMIENTO ESQUEMA TIPO PEMEX
	mttoSolSgto($id_mttoSol);
	$sgtoSgn = (is_null(@$fechaST))? 'xx':'°°';
	$colorMS = 'orange';
	if(@is_null(@$fechaET) ){ $colorMS = 'red'; } // no ha entrado a taller
	if( @$fechaSol >  '2018-06-01' ){ $colorMS = 'yellow'; } // ha entrado a taller // $fechaPg
	if( @$fechaST >  @$fechaSol ){ $colorMS = 'green'; } // salio de taller // 
	// SEGUIMIENTO ESQUEMA TIPO PEMEX




	// SEGUIMIENTO JET VAN TALLER
	$sgtoJVT = '';
	if( $id_prov == 81 And $fechaEj > '2018-06-01'){ $sgtoJVT = "<br><br><a href='https://www.jetvan.com.mx/check_jetvan/cotizador/viewerremoto.php?uuid=$id_mttoSol' target='blank' ><button>presupuesto</button></a> "   ; }
	// SEGUIMIENTO JET VAN TALLER

	echo "<td><a href='mttoSolSgto.php?id_mttoSol=$id_mttoSol&pagina=$pagina'>
			...
			<span style='background-color:$colorMS;' >$sgtoSgn</span> 
			$sgtoJVT
		  </td>";
	// SEGUIMIENTO
	...
	# PARTIDAS Y AREAS ADMINISTRATIVAS
