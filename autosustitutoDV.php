...
echo "<h2>REGISTRAR DEVOLUCION A <span style='color:red;'>JETVAN</span> VIRTUAL</h2>";
echo "<table>";
...
if (isset($_POST['actualizar']) 
	...
	{ 
		// INICIO Formatear y limpiar datos
			$fechaF = 	mysqli_real_escape_string($dbd2, $_POST['fechaF']);
			$horaF 	= 	mysqli_real_escape_string($dbd2, $_POST['horaF']);
		...
		// INICIO Update BD
			...
			$sql_sust_V = "	INSERT INTO  `jetvantlc`.`sustitutoVirtual` 
							(`id_sust_obs`, `id_sust`, `tipoMV`, `obs`, `capturo`)
							VALUES  (NULL, '$id_sustituto', '1', '$obs', '$capturoDV') ";
		...
		if($sust_R && $sust_RV)
			{ 
				echo "<br><h2>REGISTRO DE DEVOLUCION VIRTUAL EXITOSO</h2><br>";
				$actualizado = 'si';
			}
...
echo "<h3>FORMULARIO REGISTRAR DEVOLUCION VIRTUAL</h3>";
...
<form action=" " method="POST">
	<table>
		<tr>
			<th colspan=2>FORMULARIO REGISTRAR DEVOLUCION VIRTUAL</th>
		</tr>
		
		<tr>
			<td>FECHA</td>
			<td>
				<input type="date" name="fechaF" value="<?php echo  date("Y-m-d"); ?>"  required >
		...
		<tr>
			<td>HORA</td>
			<td>
				<input type="time" name="horaF" value="<?php  echo date("H:i:s"); ?>" >
			</td>
		</tr>
...
<?php  /////////////////////////////////////////////TERMINA CUENTA CARACTERES ?>
		...
		<tr>
			<td colspan=2 align=center><INPUT id="gobutton" TYPE="submit" NAME="actualizar" VALUE="CONCLUIR SUSTITUTO"></td>
		</tr>
