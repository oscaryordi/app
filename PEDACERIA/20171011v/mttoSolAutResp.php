...
$res_ejec 		= mysql_query($sql_ejec);
$matriz_ejec 	= mysql_fetch_array($res_ejec);
...
if (isset ($_POST['enviar'])) 
{
	...
	if ($_POST['id_mttoSol']!='') 
	{
		$id_mttoSol 	= mysql_real_escape_string($_POST['id_mttoSol'],$conectar);
		$autorizadoS 	= mysql_real_escape_string($_POST['autorizadoS'],$conectar);
		$obsA	   		= mysql_real_escape_string(strtoupper(trim($_POST['obsA'])),$conectar);
		...
		$sqlCanR 	= mysql_query($sqlCan);
		$matrizCR 	= mysql_fetch_array($sqlCanR);
...
/**/		$sql_MUp = "UPDATE  mttoSol SET 
			...
			$R_MUp	= mysql_query($sql_MUp,$conectar); 
			...
			if($R_MUp)
				$anadido = "Registrada ".mysql_affected_rows()." respuesta"; // Mensaje confirmación de añadido un Registro a la Base de Datos
			else
				$anadido = "ERROR AL REGISTRAR: ".mysql_error();
