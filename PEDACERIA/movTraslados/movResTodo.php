...
$cuenta_gps 		= " SELECT id_movFor FROM mov_traslados WHERE capturo = '$id_usuario' AND borrar = 0";
}

if($_SESSION["movForaneo"] > 0){
$cuenta_gps 		= " SELECT id_movFor FROM mov_traslados WHERE borrar = 0 ";	
}
...
for($i=$min_mostrar; $i <= ($max_mostrar); $i++) 
	{
		...
		echo "<a href='movResTodo.php?pagina=$i' style='color:$color;text-decoration: none; padding: 0px 7px; margin: 0px 2px; background-color: #F8F8F8 ;' >$i</a> ";
	}
