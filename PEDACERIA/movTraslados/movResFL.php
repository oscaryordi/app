...
INDIQUE NUMERO DE FACTURA -> <input type='text' name='factura' maxlength="20" >
<!--
Fecha Inicio->
...
$cuenta_gps 		= " SELECT id_movFor FROM mov_traslados WHERE capturo = '$id_usuario' AND id_prov = '$id_prov'  AND facturaT = '$factura' ";
}

if($_SESSION["movForaneo"] > 0){
$cuenta_gps 		= " SELECT id_movFor FROM mov_traslados WHERE id_prov = '$id_prov' AND facturaT = '$factura' ";	
}
...
// SI CONSULTA EJECUTIVO
	$sql_movFor = 'SELECT * '
        . ' FROM mov_traslados '
        . "  WHERE capturo = '$id_usuario'  AND id_prov = '$id_prov' AND facturaT = '$factura'  "
        . ' ORDER BY '
        . ' id_movFor '
        . ' DESC '
...
// SI CONSULTA SUPERVISOR
	$sql_movFor = 'SELECT * '
        . ' FROM mov_traslados '
        . "  WHERE id_prov = '$id_prov' AND facturaT = '$factura'  "
        . ' ORDER BY '
        . ' id_movFor '
        . ' DESC '
        . " LIMIT $pagina_1, $rxpag " ;
...
for($i=$min_mostrar; $i <= ($max_mostrar); $i++) 
	{
		...
		echo "<a href='movResProv.php?pagina=$i&consultar=consultar&id_prov=$id_prov' style='color:$color;text-decoration: none; padding: 0px 7px; margin: 0px 2px; background-color: #F8F8F8 ;' >$i</a> ";
	}
