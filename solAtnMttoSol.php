...
if(isset($_GET['id_solAtn'])){
$id_solAtn = mysqli_real_escape_string($dbd2, $_GET['id_solAtn']);
}else{
$id_solAtn = mysqli_real_escape_string($dbd2, $_POST['id_solAtn']);	
}
//ECHO   "id_solAtn".$id_solAtn."<BR>";
	...
	$kmSAtn 	= $row['km'];
	$descripcionSAtn = $row['descripcion'];
	$ubicacionSAtn = $row['ubicacion'];
	$sa1 		= $row['sa1'];
	$sa2 		= $row['sa2'];
	$sa3 		= $row['sa3'];
	$sa4 		= $row['sa4'];
...
if(isset($_POST['DatosM']))
	{
		...
		&& $siga == 1 
		)
			{
				...
				//$id_solAtn =mysqli_real_escape_string($dbd2, $_POST['id_solAtn'] );
				ECHO   "id_solAtn".$id_solAtn."DESPUES DEL POST<BR>";
				...
				if($id_contrato == 260){$supPago = '121';}

				// TIENE CREDITO
				provTieneCredito($id_prov); 
				if($credito == 1){$supPago = '898';}
				...
				{ // INICIA si no esta duplicado le damos PALANTE 
					...
					echo "<br>$sql_mttoSol<br>" ; // PARA DEBUG
					...
					if(!$sql_mttoSol_R) // SI NO HAY FALLA AL REGISTRAR
						{
							...
							echo "<h2>REGISTRO DE SOLICITUD EXITOSO CON ID <span style='color:red;'>
									$id_mttoSol</span> </h2> 
					...
					if($reembolso == 1)
					{
						$sql_reembolso = "  INSERT INTO 
											mttoSolRemb 
											(id_reembolso, id_mttoSol, nombreR, clabeR, cuentaR, 
											sucR, bancoR) 
											VALUES 
											( NULL, '$id_mttoSol', '$nombreR','$clabeR','$cuentaR', 
											'$sucR','$bancoR'
											)";
...
// MOSTRAR VARIABLES PARA CONTROL DEL PROCESO // PARA DEBUG
/*
...
<tr>
	...
	<input type="hidden" name="id_contrato"  value="<?php echo $id_cliente;?>" placeholder="Cliente" disabled >
	<input type="hidden" name="id_cliente"  value="<?php echo $id_contrato;?>" placeholder="Contrato" disabled >
	<input type="hidden" name="id_solAtn"  value="<?php echo $id_solAtn;?>" placeholder="solAtn" disabled >
	...
	<td>
		...
		<tr>
			...
			<?php echo $sa1 = ($sa1 == 1 or $sa1 == 4)?'checked':'';?>  >
...
<tr>
	...
	<input 	type="number" lang="nb" step="1" min="0" max='800000' name="km"  
			value="<?php echo $kmSAtn; ?>" placeholder="0000" required style="text-align: right;" > 0000
	</td>
</tr>
<tr>
<tr>
...
<tr>
	...
	<td>
		...
		value="<?php //echo $descripcionSAtn; ?>"  maxlength='350' 
		required ><?php echo $descripcionSAtn; ?></textarea>
		<br>
...
<tr>
	...
	<td>
		...
		value="<?php //echo $descripcionSAtn; ?>"  maxlength='250' 
		><?php echo $ubicacionSAtn; ?></textarea><br>
		Máximo 250 caracteres
	</td>
</tr>
...
<td style='vertical-align: top;'>
	...
	<input type="hidden" name="id_sucursal" 	value="<?php echo $id_sucursal;?>" >
			...
			$sql_pb = 'SELECT id_cuenta, banco, cuenta, clabe, sucursal, suspendido '
					. ' FROM '
					. ' provBanco '
					. " WHERE id_prov = '$id_prov' and suspendido = 0 ";
			...
			while($row = mysqli_fetch_assoc($resultado_pb))
			{
				...
				$checked 	= ($filas_pb==1)?'checked':'';

				echo "<tr>";
					echo "<td style='text-align:right;'>
							<input type='radio' name='id_cuenta' value='$id_cuenta' id='$id_cuenta' 
							$checked >
						  </td>";
					echo "<td><label for='$id_cuenta'>{$id_cuenta}</label></td>";
					echo "<td><label for='$id_cuenta'>{$banco}</label></td>";
					echo "<td><label for='$id_cuenta'>{$cuenta}</label></td>";
					echo "<td><label for='$id_cuenta'>{$clabe}</label></td>";
					echo "<td><label for='$id_cuenta'>{$sucursal}</label></td>";
				echo "</tr>";
			}
			echo "</table>";
			?>
