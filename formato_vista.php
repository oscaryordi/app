...
if(isset($numero_inventario) && $numero_inventario !== ''){
	global $conectar;
	$sql_obtener_formato = "SELECT * FROM `formato_inventario` WHERE `numero_inventario` = $numero_inventario ORDER BY fechaentrega DESC LIMIT 1"; 
	$sql_formato = mysqli_query($dbd2, $sql_obtener_formato );

	while($row = mysqli_fetch_assoc($dbd2, $sql_formato)){
