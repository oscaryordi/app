...
?>
<p>
<p>Consulta de Solicitudes por CLIENTE:</p>
...
$cuenta_gps;
$cuenta_gps 		= " SELECT id_mttoSol FROM mttoSol WHERE id_cliente = '$id_cliente' ";
...
echo "<p>RESUMEN DE MANTENIMIENTO X CLIENTE :</p>";
echo "<h2> $nombre </h2>";
...
if(isset($_GET['id_cliente'])) // COLOCADO POR PROBLEMA CON ID_CLIENTE AL INFORMAR PARTIDAS
{
	$id_cliente = $_GET['id_cliente'];
}
else{
...
for($i=$min_mostrar; $i <= ($max_mostrar); $i++) 
	{
		...
		echo "<a href='?pagina=$i&id_cliente=$id_cliente' style='color:$color;text-decoration: none; padding: 0px 7px; margin: 0px 2px; background-color: #F8F8F8 ;' >$i</a> ";
	}
