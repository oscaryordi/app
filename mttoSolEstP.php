...
<p>
<h3>PROVEEDORES: Consultar periodo:</h3>
...
if(isset($_GET['consultar']) || isset($_GET['fechainicio'])){// INICIA PROCESO DE FORMULARIO 
...
// VALIDA FORMATO DE FECHA
	...
	$cuenta_gps 		= "	SELECT COUNT(DISTINCT(id_prov)) sontantos 
							FROM `mttoSol`  
	...
	// echo $cuenta."<br>";
	// echo $paginas."<br>";
	echo "<h2>SOLICITUDES POR PROVEEDOR</h2>";
	echo "PERIODO DEL <b>$fechainicio</b> AL <b>$fechafinal</b><br>";
	echo "".$paginas_entero." PAGINAS,  $cuenta PROVEEDORES<br>";
	...
	*/
	...
	// SI CONSULTA GERENTE
		$sql_MSU = 'SELECT p.razonSocial nombre, 
					count( m.id_mttoSol ) solicitudes, 
					sum( m.importe ) suma ,
	...
	while($row = mysqli_fetch_assoc($sql_MSUR))
		...
		// INICIO poner renglon resultados
			...
			echo "<td><a
			href='mttoSolResSupProv.php?id_prov=$id_prov' 
			>{$nombre}</a></td>";
	...
	for($i=$min_mostrar; $i <= ($max_mostrar); $i++) 
		{
			...
			echo "<a href='mttoSolEstP.php?pagina=$i&fechainicio=$fechainicio&fechafinal=$fechafinal&cuantosR=$cuantosR&consultar=consultar' 
