...
//	echo "<p>RESUMEN DE CONTRATOS X EJECUTIVO :</p>";
	...
	// INICIO poner renglon resultados
		...
		echo "<td>Alan-><a href='clienteindexuno.php?id_cliente=$id_cliente'>
			{$id_alan}</a> ::: Numero->{$numero} ::: Alias->{$aliasCto} :::</td>";
		echo "<td> $fechainicioCTO </td>";
		echo "<td> $fechafinCTO </td>";
	...
	for($i=$min_mostrar; $i <= ($max_mostrar); $i++) 
		{
			...
			echo "<a href='?pagina=$i&ejecutivoID=$ejecutivoID' style='color:$color;text-decoration: none; padding: 0px 7px; margin: 0px 2px; background-color: #F8F8F8 ;' >$i</a> ";
		}
